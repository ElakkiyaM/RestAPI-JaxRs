package com.ind.restapi.messenger.resources;

import java.util.List;

import javax.ws.rs.Consumes;
import javax.ws.rs.DELETE;
import javax.ws.rs.GET;
import javax.ws.rs.POST;
import javax.ws.rs.PUT;
import javax.ws.rs.Path;
import javax.ws.rs.PathParam;
import javax.ws.rs.Produces;
import javax.ws.rs.core.MediaType;

import com.ind.restapi.messenger.model.Profile;
import com.ind.restapi.messenger.service.ProfileService;

@Path("/profiles")
@Consumes(MediaType.APPLICATION_JSON)
@Produces(MediaType.APPLICATION_JSON)
public class ProfileResource {
	
	ProfileService prflsvc = new ProfileService();
	
	@GET
	public List<Profile> getAllProfiles() {
		return prflsvc.getAllProfiles();
	}
	
	@GET
	@Path("/{profileName}")
	public Profile getProfile(@PathParam ("profileName") String profileName) {		
		return prflsvc.getProfile(profileName);
	}
	
	@POST
	public Profile addProfile(Profile profile) {
		return prflsvc.addProfile(profile);
	}
	
	@PUT
	@Path("/{profileName}")
	public Profile updateProfile(@PathParam("profileName") String profileName, Profile profile) {
		profile.setProfileName(profileName);
		return prflsvc.updateProfile(profile);
	}
	
	@DELETE
	@Path("/{profileName}")
	public String removeProfile(@PathParam("profileName") String profileName) {
		return prflsvc.removeProfile(profileName);
	}	

}
